  ...
  render() {
    ...
    const options = Object.keys(currentOptions).map( function(k) {
      return (  <View key={k} style={{margin:10}}>
        ...
        <Animbutton countCheck={_this.state.countCheck} onColor={"#FF3E71"} effect={"tada"} _onPress={(status) => _this._answer(status,k)} text={currentOptions[k]} />
    ...
    return (
      ...
      <View>
        ...
        <View style={Style.btBox}>
          <Button bordered onPress={() => this.prev()} style={Style.buttonPrev}>
            <Text style={Style.titleBt}>Sebelumnya</Text>
          </Button>
          <View style={{margin:15}} />
            <Button rounded onPress={() => this.next()} style={Style.buttonNext}>
